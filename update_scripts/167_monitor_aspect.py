#!/usr/bin/env python3
"""
167. update_scripts/167_monitor_aspect.py
----------------------------------------------------------------------------
aspect_ratio влияет на рендер главного экрана (MonitorPage):
  • gridStyle: колонки minmax(0,1fr), строки auto
  • cellAspect из setData.aspect_ratio
  • CameraCard/CameraEmpty в обёртке .aspect-wrap

ЗАПУСК: python update_scripts/167_monitor_aspect.py
"""
...
def main():
    ...
    new = "    gridStyle.gridTemplateColumns = `repeat(${setData.max_columns}, minmax(0, 1fr))`  // PATCH-167"
    if old in content:
...
"""
    ...
    if old in content and "cellAspect" not in content:
        content = content.replace(old,
            "  // PATCH-167: пропорции ячейки из формата набора\n"
            "  const cellAspect = ((setData && setData.aspect_ratio) || '16:9').replace(':', ' / ')\n" + old, 1)
        m += 1
        print("  [OK] cellAspect")

    # 4. CameraCard в .aspect-wrap
    old = """                return (
                  <CameraCard
                    key={camera.id}
                    camera={camera}
    ...
    if "PATCH-167" not in css:
        css += "\n/* PATCH-167: строки auto, скролл внутри */\n.fullscreen-grid { align-content: start; overflow: auto; }\n"
    ...
    if "aspect-wrap" not in css:
        css += """
/* PATCH-167: обёртка ячейки с пропорциями набора */
.aspect-wrap {
  min-width: 0;
  min-height: 0;
  position: relative;
