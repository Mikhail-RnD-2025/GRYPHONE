#!/usr/bin/env python3
"""
210. update_scripts/210_dashboard_health_section.py
----------------------------------------------------------------------------
Расширение существующего дашборда (/status):
  • components/CameraHealth.jsx — самодостаточная секция здоровья камер
    (fetch /api/health/cameras, автообновление 5 сек, summary + таблица)
  • StatusPage.jsx — импорт + <CameraHealth /> после <Dashboard />

Ничего не откатываем (PATCH-209 не применялся). Dashboard.jsx не трогаем.

ЗАПУСК: python update_scripts/210_dashboard_health_section.py
"""
...
def create_component(root):
    print("--- создание components/CameraHealth.jsx ---")
    f = root / "frontend" / "src" / "components" / "CameraHealth.jsx"
    if f.exists():
        print("  [SKIP] уже существует")
        return True
    f.write_text(CAMERA_HEALTH, encoding="utf-8")
    print(f"  [OK] создан ({f.stat().st_size} bytes)")
    return True


def patch_status_page(root):
    print("--- StatusPage.jsx: подключение <CameraHealth /> ---")
    f = root / "frontend" / "src" / "pages" / "StatusPage.jsx"
    b = f.with_suffix(".jsx.bak-210")
    b.write_text(f.read_text(encoding="utf-8"), encoding="utf-8")
    c = f.read_text(encoding="utf-8")
    n = 0

    if "CameraHealth" not in c:
        old_imp = "import Dashboard from '../components/Dashboard'"
        if old_imp in c:
            c = c.replace(
                old_imp,
    ...
    if n == 2:
        ...
        print("  [OK] импорт + <CameraHealth /> после <Dashboard />")
        return True
    if "CameraHealth" in c and n == 0:
        print("  [OK] уже подключён")
        return True
    print(f"  [FAIL] применено {n}/2 — откат")
...
def main():
    ...
    print("=" * 76)
    print("210: секция «Здоровье камер» в существующем дашборде /status")
    print("=" * 76)
    print()

    ok = True
    ok &= create_component(root)
    ok &= patch_status_page(root)
    ...
    print()
    print("=" * 76)
    print("✅ PATCH-210 готов! Сборка + запуск:")
    print()
    print(f"  cd {root}/frontend && npm run build")
    print(f"  cd {root} && python main.py")
    print()
    print("Откройте:  http://127.0.0.1:5000/status")
    print()
    print("Ожидаемо (под существующим дашбордом):")
    print("  • 🏥 Здоровье камер + таймстамп обновления")
    print("  • 6 summary-плашек: Всего 24 / Вкл 4 / Выкл 20 / Стрим 0 / Подкл 0 / Ошибок 4")
    print("  • Фильтры: Все / Включённые / Выключенные / Стримящиеся / С ошибкой")
    print("  • Таблица 24 строки: цветные ячейки Main/Sub (tooltip с деталями и fps)")
    ...
    print(f"cd {root}")
    print("git add -A")
    print('git commit -m "feat(dashboard): camera health section on /status (PATCH-210)" \\')
    print('  -m "components/CameraHealth.jsx: self-contained health matrix" \\')
    print('  -m "summary cards + filters + color-coded main/sub states" \\')
    print('  -m "auto-refresh 5s via /api/health/cameras (PATCH-208)" \\')
    print('  -m "StatusPage: <CameraHealth /> after <Dashboard />; Dashboard.jsx untouched"')
    print("git push")
    print("=" * 76)
