#!/usr/bin/env bash
# ============================================================
#  GRYPHONE — скрипт 35: ИМПОРТ ИЗ EXCEL (обновлённый)
#  ------------------------------------------------------------
#  Обновляет import_from_excel.py с полной поддержкой всех
#  полей камеры как в cameras.json:
#    - id, name, main_url, sub_url, enabled, comment
#    - audio, location (новые поля из скрипта 27)
#
#  Что улучшено:
#    • Явная структура всех 8 полей с типами и дефолтами
#    • Очистка дефиса в начале имени (как в исходных данных)
#    • Поддержка импорта наборов из второго листа
#    • Подробное логирование каждого поля
#    • Валидация всех полей перед сохранением
#
#  Запуск:   bash 35_excel_import_v2.sh
...
# ============================================================
# Обновлённый import_from_excel.py
# ============================================================
...
if __name__ == '__main__':
    ...
    if not result['success']:
        sys.exit(1)
PYEOF_EXCEL
echo "  ✔ import_from_excel.py (все 8 полей, очистка дефисов)"

# ============================================================
# Проверка
# ============================================================
if [ ! -s "$PROJECT_DIR/import_from_excel.py" ]; then
...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "✅ Скрипт импорта обновлён"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo "📋 Все 8 полей камеры поддерживаются:"
echo "  ✅ id        — идентификатор (обязательно)"
echo "  ✅ name      — имя (очистка дефиса в начале)"
echo "  ✅ main_url  — основной поток (обязательно, очистка пробелов)"
echo "  ✅ sub_url   — субпоток (очистка пробелов)"
echo "  ✅ enabled   — включена/выключена (парсинг из любого формата)"
echo "  ✅ comment   — комментарий"
echo "  ✅ audio     — звук вкл/выкл"
echo "  ✅ location  — местоположение"
echo ""
echo "🚀 Использование:"
echo "  python import_from_excel.py cameras.xlsx"
echo ""
echo "📖 Или через веб-интерфейс:"
echo "  Настройки → Наборы → 📥 Импорт из Excel"