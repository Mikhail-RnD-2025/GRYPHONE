#!/usr/bin/env python3
"""
189. update_scripts/189_cameras_modal_form.py
----------------------------------------------------------------------------
Модальная форма редактирования камеры:
  • Вместо inline-редактирования в таблице — модальное окно
  • 7 полей: login, pass, ipaddress, port, main_url, sub_url, sub2_url
  • Предпросмотр собранного URL в реальном времени
  • Таблица показывает: ID, Name, IP:Port, Enabled, Actions

ЗАПУСК: python update_scripts/189_cameras_modal_form.py
"""
...
CAMERAS_EDITOR_JSX = r'''import { useState, useEffect } from 'react'
...
def main():
    ...
    print("=" * 76)
    print("189: модальная форма камеры с 7 полями + предпросмотр URL")
    print("=" * 76)
    print()

    b = f.with_suffix(".jsx.bak-189")
    b.write_text(f.read_text(encoding="utf-8"), encoding="utf-8")

    if CAMERAS_EDITOR_JSX.count('{') != CAMERAS_EDITOR_JSX.count('}'):
        print("  [FAIL] скобки не сбалансированы — откат")
        sys.exit(1)

    f.write_text(CAMERAS_EDITOR_JSX, encoding="utf-8")
    print("  [OK] CamerasEditor.jsx перезаписан")

    print()
    print("=" * 76)
    print("✅ Готово!")
    print()
    print("Новая форма редактирования:")
    print("  • Модальное окно вместо inline-редактирования")
    print("  • 7 полей: login, pass, ipaddress, port, main_url, sub_url, sub2_url")
    print("  • Предпросмотр собранного URL в реальном времени")
    print("  • Дополнительно: comment, location, audio")
    print()
    ...
    print(f"cd {root}")
    print("git add -A")
    print('git commit -m "ui: modal camera form with 7 RTSP parts + URL preview (PATCH-189)" \\')
    print('  -m "CamerasEditor: modal dialog instead of inline editing" \\')
    print('  -m "7 fields: login/pass/ipaddress/port/main_url/sub_url/sub2_url" \\')
    print('  -m "live preview of assembled rtsp:// URL" \\')
    print('  -m "table shows IP:Port instead of full URL"')
    print("git push")
    print("=" * 76)
