#!/usr/bin/env python3
"""
160. update_scripts/160_dragend_context_menu.py
----------------------------------------------------------------------------
Исправления:
  1. onDragEnd на draggable элементах — сброс состояния ПОСЛЕ завершения drag
     (убирает "подвисание" ghost-image)
  2. Контекстное меню (ПКМ) с опциями:
     • На камере в списке: "Добавить в набор"
     • На камере в сетке: "Убрать из набора"
     • На пустой ячейке: нет опций
     • На области сетки: "Очистить весь набор"
     • На области списка: "Добавить все видимые"

ЗАПУСК: python update_scripts/160_dragend_context_menu.py
"""
...
.sets-ctx-item:hover { background: rgba(37, 99, 235, 0.2); }
.sets-ctx-item:disabled {
  opacity: 0.4;
  cursor: not-allowed;
  background: transparent !important;
}
.sets-ctx-danger { color: #f87171; }
.sets-ctx-danger:hover:not(:disabled) { background: rgba(220, 38, 38, 0.2); }
"""
...
def main():
    ...
    print("=" * 76)
    print("160: Фикс подвисания drag + контекстное меню (ПКМ)")
    print("=" * 76)
    print()

    backup = jsx_file.with_suffix(".jsx.bak-160")
    backup.write_text(jsx_file.read_text(encoding="utf-8"), encoding="utf-8")
    print(f"  [BAK] {backup.name}")

    if SETS_JSX.count('{') != SETS_JSX.count('}') or \
       SETS_JSX.count('(') != SETS_JSX.count(')'):
        print("  [FAIL] Скобки не сбалансированы — откат")
        sys.exit(1)

    jsx_file.write_text(SETS_JSX, encoding="utf-8")
    print("  [OK] SetsPage.jsx: onDragEnd + контекстное меню")

    css = css_file.read_text(encoding="utf-8")
    if "PATCH-160" not in css:
        css_file.write_text(css + CSS_ADD, encoding="utf-8")
        print("  [OK] sets.css: стили контекстного меню")

    print()
    print("=" * 76)
    print("✅ Готово! Что нового:")
    print()
    print("1. Подвисание убрано:")
    print("   • onDragEnd срабатывает ПОСЛЕ завершения drag")
    print("   • ghost-image исчезает мгновенно")
    print()
    print("2. Контекстное меню (правый клик):")
    print("   • На камере в списке → Добавить в набор / Убрать")
    print("   • На камере в сетке → Убрать из набора")
    print("   • На пустой ячейке / в сетке → 🗑 Очистить весь набор")
    print("   • В области списка → Добавить все видимые")
    print()
